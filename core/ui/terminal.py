...
def _format_historical_timestamp(ts: str) -> str:
...
def _get_historical_metric_keys(snapshots: List[dict]) -> List[str]:
	...
	try:
		...
		return [r["metric"] for r in latest_results[:10]]
	except (json.JSONDecodeError, KeyError, IndexError):
		return []


def _calculate_historical_row(
	snap: dict, metric_keys: List[str], metric_labels: Dict[str, str]
) -> List[str]:
	"""Calculate a single row for the historical scores table."""
	score = snap["total_score"]
	color = "bold green" if score >= 70 else "yellow" if score >= 40 else "red"
	ts_display = _format_historical_timestamp(snap["timestamp"])
...
def display_historical_scores(symbol: str, profile: str, snapshots: List[dict]):
	...
	metric_keys = _get_historical_metric_keys(snapshots)
	for key in metric_keys:
		label = metric_labels.get(key, key[:5])
		table.add_column(label, justify="right")

	for snap in snapshots:
		row = _calculate_historical_row(snap, metric_keys, metric_labels)
		table.add_row(*row)
...
def _display_perf_section(stats: SessionStats):
...
def _display_endpoints_section(stats: SessionStats):
...
def _display_quality_section(stats: SessionStats):
	if not stats.data_coverage:
		return
...
def _display_resilience_section(stats: SessionStats):
	error_data = stats.to_dict()
	if stats.errors == 0 and stats.retry_attempts == 0:
		return
	err_table = Table(title="Error Topology & Resilience", box=None, show_header=False)
	...
	if stats.retry_attempts > 0:
		...
		color = "green" if retry_rate >= 80 else "yellow" if retry_rate >= 50 else "red"
		err_table.add_row(
			"Retry Success Rate",
...
def _display_efficiency_section(stats: SessionStats):
	eff_table = Table(
	...
	if (db_growth := res_data.get("db_growth_bytes", 0)) > 0:
		eff_table.add_row("DB Growth", f"+{db_growth / 1024:.1f} KB")

	if (snapshots := res_data.get("db_snapshots", 0)) > 0:
...
def display_run_summary(stats: SessionStats):
	...
	console.print("\n")
	_display_perf_section(stats)
	_display_endpoints_section(stats)
